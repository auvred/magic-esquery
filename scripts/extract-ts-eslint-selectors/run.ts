...
const newlyGeneratedSelectors = fs
  .readFileSync(path.join(eslintPluginDir, 'listeners'), 'utf8')
  .split('\n')
...
const newSelectors = newlyGeneratedSelectors.filter(selector => {
...
tsEslintSelectors.push(`# ${tsEslintRev}`, ...newSelectors, '')
